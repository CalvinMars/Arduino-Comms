...
void xbee::xbee_in_array() {
	if (Serial2.available()>0) {
		bufferin[count] = Serial2.read()&0xFF;
		if (count>2) {
...
void xbee::sendata(uint8_t* data) {
  size_t data_length = sizeof(data)/sizeof(uint8_t);
	char buff[50];
	bufferout[0] = 0x7E; bufferout[1] = 0x00;
	bufferout[2] = data_length+14;
	bufferout[3] = 0x10; bufferout[4] = 0x01;
	bufferout[13] = 0xFF; bufferout[14] = 0xFE;
	bufferout[15] = 0x00; bufferout[16] = 0x00;
	
//	data.toCharArray(buff,data_length+1);
	
	int i = 17;
	while (data_length+17>i){
		bufferout[i] = data[i - 17];
		i++;
	}
	bufferout[data_length+17]=checksum(3, data_length + 16, 'O');
//	digitalWrite(wakeperro, HIGH);
//	delay(10);
//	digitalWrite(wakeperro, LOW);
//	delay(10);
	for (size_t i = 0; i < data_length + 18; i++) Serial2.write(bufferout[i] & 0xFF);
}
